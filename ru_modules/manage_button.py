...
from ru_modules.helper_funcs.strings import back_button, manage_button_str_1, manage_button_str_2, create_button_button, \
...
class ButtonEdit(object):
    ...
    def edit_button_finish(self, bot, update, user_data):
        ...
        for key, value in user_data["button_info"].items():
            if "_files" in key:
                if user_data["content_id"] in button_info[key]:
                    button_info[key].remove(user_data["content_id"])
        ...
        if update.message.text:
            ...
            photo_file = update.message.photo[-1].get_file().id

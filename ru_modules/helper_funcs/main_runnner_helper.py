...
from ru_modules import ALL_MODULES
from ru_modules.helper_funcs.auth import if_admin, initiate_chat_id, register_chat
from ru_modules.helper_funcs.misc import paginate_modules, LOGGER, EqInlineKeyboardButton
...
for module_name in ALL_MODULES:
    imported_module = importlib.import_module("ru_modules." + module_name)
...
def send_admin_user_mode(bot, chat_id, text, keyboard=None):
    ...
    keyboard = keyboard + [[EqInlineKeyboardButton(text="Назад в режим админа", callback_data="turn_user_mode_off")]]
    bot.send_message(chat_id=chat_id,
                     text=text,
                     parse_mode=ParseMode.MARKDOWN,
...
def button_handler(bot: Bot, update: Update):
    ...
    bot.send_message(chat_id=update.callback_query.message.chat_id,
                     text="Нажми '🔙 Назад ', чтобы вернуться в меню ", reply_markup=InlineKeyboardMarkup(
            buttons))

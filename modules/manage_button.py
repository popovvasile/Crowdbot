...
class ButtonEdit(object):
                    ...
                    update.message.reply_text(
                        ...
                        reply_markup=InlineKeyboardMarkup([[
                            InlineKeyboardButton(text="EDIT",
                                                 callback_data="btn_edit_{}__{}".format(descr,
                                                                                        update.message.text))]])
                    )
                ...
                for filename in button_info["audio_files"]:
                    with open(filename, 'rb') as file:
                        update.message.reply_audio(
                            ...
                            reply_markup=InlineKeyboardMarkup([[
                                InlineKeyboardButton(text="EDIT",
                                                     callback_data="btn_edit_{}__{}".format(
    ...
    def edit_button(self, bot, update, user_data):
        bot.delete_message(chat_id=update.callback_query.message.chat_id,
                           message_id=update.callback_query.message.message_id)
        content_data = update.callback_query.data.replace("btn_edit_", "").split("__")
    ...
    def edit_button_finish(self, bot, update, user_data):
        ...
        if "audio_files" in button_info:
            ...
            os.remove(user_data["content_id"])

        elif "video_files" in button_info:
            button_info["video_files"].remove(user_data["content_id"])
            os.remove(user_data["content_id"])

        elif "document_files" in button_info:
            button_info["document_files"].remove(user_data["content_id"])
            os.remove(user_data["content_id"])

        elif "photo_files" in button_info:
            button_info["photo_files"].remove(user_data["content_id"])
            os.remove(user_data["content_id"])

        elif "descriptions" in button_info:
        ...
        if update.message.text:
            ...
            button_info["audio_files"].append(custom_path)
            ...
            button_info["audion_files"].append(custom_path)
            ...
            button_info["document_files"].append(custom_path)
            ...
            button_info["video_files"].append(custom_path)
...
BUTTON_EDIT_FINISH_HANDLER = ConversationHandler(
    ...
    states={
        ...
        EDIT_FINISH: [MessageHandler(Filters.text, ButtonEdit().edit_button_finish, pass_user_data=True),
                      CallbackQueryHandler(callback=ButtonEdit().back,
                                           pattern=r"cancel_edit_button", pass_user_data=True),
                      ],
    },

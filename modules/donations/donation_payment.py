...
class DonationBot(object):
    ...
    def start_donation(self, update, context):
        ...
        current_user_mode = (
                user_mode_table.find_one(
                    {"bot_id": context.bot.id,
                     "user_id": update.effective_user.id})
                or {"user_mode": False})

        if (donation_request.get("donate") is not None
                and donation_request.get("donate") != {}):
            context.user_data['to_delete'].append(
                context.bot.send_message(
                    chat_id=update.callback_query.message.chat.id,
                    text=donation_request["donate"]["description"]))

            context.user_data['to_delete'].append(
                context.bot.send_message(
                    chat_id=update.callback_query.message.chat.id,
                    text=context.bot.lang_dict["pay_donation_str_1"]))

            context.user_data['to_delete'].append(
                context.bot.send_message(
                    chat_id=update.callback_query.message.chat.id,
                    text=context.bot.lang_dict["pay_donation_str_2"].format(
            ...
            try:
                if current_user_mode["user_mode"] is True:
                    ...
                    context.bot.send_message(
                        chat_id=update.callback_query.message.chat.id,
                        text=context.bot.lang_dict["pay_donation_str_4"],
                        reply_markup=InlineKeyboardMarkup([admin_keyboard]))
                elif if_admin(update, context):
                    admin_keyboard = [
                        InlineKeyboardButton(
                            text=context.bot.lang_dict[
                                "allow_donations_button"],
                    ...
                    context.bot.send_message(
                        chat_id=update.callback_query.message.chat.id,
                        text=context.bot.lang_dict["allow_donation_text"],
                        reply_markup=InlineKeyboardMarkup([admin_keyboard]))
                else:
                    admin_keyboard = [
                        InlineKeyboardButton(
                ...
                admin_keyboard = [
                    InlineKeyboardButton(
                        text=context.bot.lang_dict["allow_donations_button"],
                        callback_data="allow_donation"),
                    InlineKeyboardButton(
                        text=context.bot.lang_dict["back_button"],
                        callback_data="help_back")]
    ...
    def execute_donation(self, update, context):
        ...
        try:
            ...
            update.message.reply_text(
                text=context.bot.lang_dict["pay_donation_str_5"],
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton(
            ...
            return EXECUTE_DONATION
        donation_request = chatbots_table.find_one(
            {"bot_id": context.bot.id})["donate"]
        ...
        context.bot.sendInvoice(chat_id, title, description,
    ...
    def precheckout_callback(self, update, context):
        ...
        context.bot.answer_pre_checkout_query(pre_checkout_query_id=query.id,
                                              ok=True)
        return ConversationHandler.END
